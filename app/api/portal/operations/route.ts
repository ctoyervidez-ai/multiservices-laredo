...
export async function GET(request: Request) {
  try {
    ...
    const [inquiries, metrics, submissions, notifications] = await Promise.all([
      db.prepare(`SELECT id, reference, full_name AS fullName, company, phone, email, need, message, status, created_at AS createdAt
...
export async function POST(request: Request) {
  ...
  try {
    const data = await jsonWithLimit<{ action?: string; id?: string; status?: string }>(request, 4096);
    ...
    if (data.action === 'retry_notifications') {
      ...
      const [updated] = await db.batch([
        db.prepare('UPDATE inquiries SET status = ?, updated_at = ? WHERE id = ? AND tenant_id = ?').bind(data.status, now, data.id, context.tenantId),
        db.prepare(`INSERT INTO audit_logs (id, tenant_id, actor_id, actor_email, action, entity_type, entity_id, summary, created_at)
          SELECT ?, ?, ?, ?, 'inquiry.status_changed', 'inquiry', ?, ?, ? WHERE EXISTS (SELECT 1 FROM inquiries WHERE id = ? AND tenant_id = ?)`)
      ...
      if (!updated.meta.changes) return respond({ error: 'No encontramos el prospecto.' }, 404);
