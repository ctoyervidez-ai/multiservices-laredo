import { authenticatePortalAdmin, PortalAuthError, type PortalAuthInput } from '@/lib/portal-auth';
...
export async function POST(request: Request) {
  ...
  let input: PortalAuthInput;
  try {
    input = await jsonWithLimit<PortalAuthInput>(request, 4 * 1024);
  } catch (error) {
  ...
  try {
    const result = await authenticatePortalAdmin(request, input);
    return respond({ ok: true, user: result.identity }, 200, { 'set-cookie': result.setCookie });
  } catch (error) {
    ...
    console.error('portal_login_route_failed');
